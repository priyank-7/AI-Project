...
from wumpus_colour_and_move import *
...
class HybridWumpusAgent(Explorer):
    ...
    def make_percept_sentence(self, raw_percepts):
        raw_percepts_manual = generate_percept_sequence()
        sentence = axiom_generator_percept_sentence(self.time,raw_percepts_manual)
    ...
    def add_temporal_axioms(self):
        ...
        if self.verbose:
            ax_so_far = len(list(axioms))
            print("           number of location_OK axioms:         {0}".format(ax_so_far))
        axioms = list(axioms) + list(generate_breeze_percept_and_location_axioms(self.time, 1, self.width, 1, self.height))
    ...
    def update_unvisited_locations(self):
        ...
        if self.verbose:
            ...
            for vis_loc in already_visited:
                display_env.add_thing(Proposition(expr('~Vis'),'T'),vis_loc)
            start_time = time()
